...
- [[#languages-and-about-languages-on-the-beam][Languages, and about languages, on the BEAM]]
  - [[#overview][Overview]]
  - [[#57-languages-on-the-beam][57 Languages on the BEAM]]
...
** 57 Languages on the BEAM

First the  1 Erlang itself,
 then the  6 main [[#lisps][Lisp-influenced]] languages,
 then the 25 [[#statically-typed-languages][statically-typed]] languages,
 then the 25 [[#etc][others]].  That's
          57 languages in all.

At the December 2025 update, there were too many lisps implemented in erlang and (mostly) elixir in github to count.  There are also quite a few language projects I missed earlier.  Projects which haven't been updated since 2018 I haven't always included.  Many languages I didn't include here are in  [[https://github.com/stars/michallepicki/lists/beam-languages][Michał Łępicki's beam languages]] list.
...
"A typed language for long-running agent systems on BEAM: effect-row tracking, auditable tool effects, typed actors, and OTP supervision."  Rust-implemented compiler emitting Erlang source; also ships an LSP server and an MCP server exposing compiler queries (type inference, effect rows, actor protocols) to LLM agents.

- https://github.com/no-materials/hird
